 ...
 */
public class AvroSpecificRecordReader<T extends SpecificRecord> implements
        RecordReader<NullWritable, AvroSpecificRecordWritable>, JobConfigurable {
  private static final Logger LOG = LoggerFactory.getLogger(AvroSpecificRecordReader.class);

  final private org.apache.avro.file.FileReader<T> reader;
  ...
  public AvroSpecificRecordReader(JobConf job, FileSplit split, Reporter reporter) throws IOException {
    this.jobConf = job;
    Schema latest;
    ...
    SpecificDatumReader<T> sdr = new SpecificDatumReader<>();

    if(latest != null) {
      sdr.setExpected(latest);
    }
    ...
    if (split.getLength() == 0) {
      ...
      this.reader = new DataFileReader(new FsInput(split.getPath(), job), sdr);
...
/*
  ...
  @Override
  public boolean next(NullWritable nullWritable, AvroSpecificRecordWritable<T> record) throws IOException {
  ...
  @Override
  public boolean next(NullWritable nullWritable, AvroSpecificRecordWritable record) throws IOException {
    if(isEmptyInput || !reader.hasNext() || reader.pastSync(stop)) {
      return false;
    }
  ...
  @Override
  public AvroSpecificRecordWritable createValue() {
    return new AvroSpecificRecordWritable<T>();
  }
